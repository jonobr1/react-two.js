...
function Scene() {
  ...
  useFrame((elapsed) => {
    if (path.current) path.current.rotation = elapsed * 0.5;
    if (points.current) points.current.rotation = -elapsed * 0.3;
    if (circle.current) circle.current.scale = Math.sin(elapsed) * 0.25 + 1;
    if (rectangle.current) rectangle.current.rotation = elapsed * 0.2;
    ...
    if (ellipse.current) {
      ...
      if (ellipse.current.dashes) {
        ellipse.current.dashes.offset = -50 * elapsed;
      }
    }
    if (line.current) line.current.rotation = elapsed * 0.4;
    if (polygon.current) polygon.current.rotation = elapsed * 0.1;
    if (star.current) {
    ...
    if (sprite.current) sprite.current.rotation = elapsed * 0.3;
